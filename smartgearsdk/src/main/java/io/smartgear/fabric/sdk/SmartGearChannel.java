 ...
 */
class SmartGearChannel {

    private static final Log log = LogFactory.getLog(SmartGearChannel.class);
...
//        client.setUserContext(org.getUser(org.getUsername()));
     ...
     * Peer加入频道
     */
    Map<String, String> joinPeer(SmartGearPeer peer) throws InvalidArgumentException, ProposalException {
     ...
     * 在指定频道内根据transactionID查询区块
     */
    Map<String, String> queryTransitionByTransactionID(String txID) throws InvalidArgumentException, ProposalException, IOException,Exception {
        return queryTransitionINfo(channel.queryTransactionByID(txID));
    }
    /**
     * 解析交易信息的对象
     */
    Map<String,String> queryTransitionINfo(TransactionInfo transition)throws  Exception{
        Map<String,String> result = new HashMap<>();

//         = Hex.encodeHexString(transition.getProcessedTransaction().getTransactionEnvelope().getPayload());


//        byte[] input = transition.


//        String testStr = printableString(new String(input));
        int testStr = transition.getValidationCode().getNumber();



        System.out.println(transition.getEnvelope());
    ...
    private Map<String, String> execBlockInfo(BlockInfo blockInfo) throws IOException, InvalidArgumentException {
        ...
        for (EnvelopeInfo info : blockInfo.getEnvelopeInfos()) {
            ...
            envelopeJson.put("timestamp", info.getTimestamp().getTime());  //获取时间戳
            ...
            log.debug("timestamp = " + info.getTimestamp().getTime()); //获取时间戳
